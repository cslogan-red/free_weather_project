...
// weather lookup by lat, lng string get
APP.get( '/weather/lookup/:lat,:lng', ( req, res) => {

    if ( req && req.params && req.params.lat && req.params.lng) {
        WEATHER._getWeatherResultAsync( req.params.lat, req.params.lng)
        .then( doc => {
            res.status( 200).send( doc)
        })
        .catch( error => {
